import {div, h1, hr, input, label, MainDOMSource, makeDOMDriver, VNode} from '@cycle/dom';
...
function addLink(value: Edge) {
  ...
  while (nodes.length <= nodeMax) {
    const newNode =
        Object.assign({}, existingNode, {id: nodes.length.toString()});
    nodes.push(newNode);
  }
...
function run_d3() {
  ...
  const force =
      d3.forceSimulation()
          ...
          .force(
              'link',
              d3.forceLink(links).id((d: Node|{}) => isNode(d) ? d.id : ''))
  ...
  let link: d3.Selection<SVGLineElement, Link, d3.BaseType, {}> =
      svg.append('g')
          .attr('stroke', '#000')
          .attr('stroke-width', 1.5)
          .selectAll('.link');

  let node: d3.Selection<SVGCircleElement, Node, d3.BaseType, {}> =
      svg.append('g')
          .attr('stroke', '#fff')
          .attr('stroke-width', 1.5)
          .selectAll('.node');
  ...
  function ticked() {
    link.attr('x1', (d: Link) => isNode(d.source) ? d.source.x || 0 : 0)
  ...
  function render() {
    ...
    link.exit().remove();
    link = link.enter().append<SVGLineElement>('line').merge(link);
